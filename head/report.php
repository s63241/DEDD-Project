...
//Get question date
$termYear = explode("/",$termnow);
mysql_select_db($database_conn, $conn);
$q = "select * from tbl_date_q where dq_name like '%".$termYear[1]."' ";
...
mysql_select_db($database_conn, $conn);
$query_sumg1 = "SELECT SUM(s_score) AS TOTALSCORE,s_p_id_2 "
        . "FROM tbl_score "
        . "WHERE ref_p_id='$p_id' "
        . "AND s_group_no=1 "
...
mysql_select_db($database_conn, $conn);
$q = sprintf("
  SELECT * FROM tbl_personal as p left join tbl_position as s ON p.ref_po_id=s.po_id WHERE p.p_id = %d", GetSQLValueString($s_p_id_2, "int"));
$result = mysql_query($q, $conn) or die(mysql_error());
$assessor = mysql_fetch_assoc($result);
$assessorNumRow = mysql_num_rows($result);
//end
...
$assessorName = ($assessorNumRow>0?($assessor['p_firstname'] . $assessor['p_name'] . ' ' . $assessor['p_lastname']):'');
$tb1 = $tb1 . '</table>
<table width="100%" >
    ...
    <tr style="border:none;">
        <td style="border:none;"><strong>ชื่อผู้ประเมิน</strong> '.($assessorNumRow>0?$assessorName:'').'</td>
        <td style="border:none;"><strong>ตำแหน่ง</strong> '.($assessorNumRow>0?$assessor['po_name']:'').'</td>
        
    </tr>
</table>
<table id="bg-table" width="100%" style="border-collapse: collapse;margin-top:8px;">
  <tr style="">
    <td style="" colspan="3"><center>แบบประเมินที่ 1 ผลสัมฤทธิ์ของงาน 70 คะแนน พิจารณาจากเกณฑ์ที่สำนักฯให้</center></td>
  </tr>
  <tr style="">
