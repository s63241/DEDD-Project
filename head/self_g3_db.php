...
$skey = $ref_p_id.$ref_dq_id.$mdate.$hdate; 


 $check ="SELECT * FROM tbl_score  WHERE ref_p_id=$ref_p_id AND ref_dq_id='$ref_dq_id' AND  s_group_no=3 ";
...
if($num > 0)
{
			...
			echo "window.location='report.php?p=report&term=$ref_dq_id&p_id=$ref_p_id';";
			//echo "window.location ='index2.php?p=report'; ";
			echo "</script>";
...
$sql2 = "UPDATE tbl_score SET 
		skey='$skey' WHERE ref_p_id=$ref_p_id AND ref_dq_id='$ref_dq_id'";
...
if($result){
			...
			// echo "window.open(
  	// 			 'index.php?p=report&term=$ref_dq_id&per_id=$ref_p_id',
  	// 			'_blank');";
			echo "window.location='report.php?p=report&term=$ref_dq_id&p_id=$ref_p_id';";
			echo "</script>";
	  }
